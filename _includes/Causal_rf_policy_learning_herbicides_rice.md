# Causal Random Forest and Policy Learning: Targeting Weed Management
Options in Rice Example
LCAS Team

- [<span class="toc-section-number">1</span>
  Introduction](#introduction)
- [<span class="toc-section-number">2</span> Loading
  dataset](#loading-dataset)
- [<span class="toc-section-number">3</span> Descriptive statistics and
  graphics](#descriptive-statistics-and-graphics)
  - [<span class="toc-section-number">3.1</span> Mean
    comparisons](#mean-comparisons)
- [<span class="toc-section-number">4</span> Causal Random Forest and
  Policy Learning](#causal-random-forest-and-policy-learning)
  - [<span class="toc-section-number">4.1</span> Causal random
    forest](#causal-random-forest)
  - [<span class="toc-section-number">4.2</span> Policy
...
potential effectiveness of the practices.

We then use a debiased robust estimator in a policy tree optimization
(Athey and Wager 2021) to generate optimal recommendations in the form
of agronomic practices that maximize potential yield gains.

We use weed management as an exemplar on how to use causal random forest
estimator and policy tree algorithm in generating individual specific
recommendations.
...
To understand the extent of herbicide use, the figure show the number of
farmers using different weed management practices.
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-3-1.png)
...
Before diving into the causal random forest model, one needs to conduct
the conventional mean comparison.
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-4-1.png)
...
```
    ...
    Variable importance: 
        1     2     3     4     5     6     7     8     9 
    0.093 0.006 0.003 0.001 0.003 0.014 0.364 0.359 0.156 
...
```
                                                                   ...
                                                                   estimate
    Manual_Bispyribac Sodium - Manual_No herbicide             -0.199122326
    Manual_Bispyribac sodium  Oxadiargyl - Manual_No herbicide  0.191685558
    Manual_Oxadiargyl - Manual_No herbicide                    -0.005945171
    Manual_Pretilachlor - Manual_No herbicide                   0.052460187
                                                                  std.err
    Manual_Bispyribac Sodium - Manual_No herbicide             0.18993445
    Manual_Bispyribac sodium  Oxadiargyl - Manual_No herbicide 0.05052365
    Manual_Oxadiargyl - Manual_No herbicide                    0.01462194
    Manual_Pretilachlor - Manual_No herbicide                  0.25899604
                                                                                                                 contrast
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-5-1.png)
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-6-1.png)
...
```
    ...
    Variable splits: 
    (1) split_variable: market_distance  split_value: 7.5 
      (2) split_variable: latitude  split_value: 25.12 
        (4) split_variable: flood_severity_num  split_value: 1 
          (8) * action: 5 
          (9) * action: 2 
      ...
      (3) split_variable: longitude  split_value: 84.46 
        (6) split_variable: drought_severity_num  split_value: 2 
          (12) * action: 5 
          (13) * action: 3 
        (7) split_variable: market_distance  split_value: 12 
          (14) * action: 2 
          (15) * action: 5 
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-6-2.png)
...
```
    ...
    (1) split_variable: market_distance  split_value: 7.5 
      (2) split_variable: latitude  split_value: 25.12 
        (4) split_variable: latitude  split_value: 21.668 
          (8) split_variable: latitude  split_value: 20.725 
            (16) * action: 5 
            (17) * action: 4 
          (9) split_variable: flood_severity_num  split_value: 1 
            (18) * action: 5 
            (19) * action: 2 
        (5) split_variable: longitude  split_value: 85.823 
          (10) split_variable: longitude  split_value: 85.338 
            (20) * action: 5 
            (21) * action: 2 
          (11) split_variable: longitude  split_value: 87.029 
            (22) * action: 5 
            (23) * action: 3 
      (3) split_variable: longitude  split_value: 84.46 
...
![](Causal_rf_policy_learning_herbicides_rice_files/figure-commonmark/unnamed-chunk-6-3.png)
