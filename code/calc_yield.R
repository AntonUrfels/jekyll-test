...
calc_yield <- function(df) {
  ...
  i <- 100 # for quintal #TODO add manual choice for different units here. Potentially integrate like LLU into survey.
  df$total_production_lp_ton <- df$total_production_lp / i
  df$total_production_farm_ton <- df$total_production_lp / i
