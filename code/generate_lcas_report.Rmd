...
Unit: **`r countrycode::codelist[which(countrycode::codelist$iso.name.en=="India"),]$iso4217c` / 100 kg**
...
library(ggplot2)
library(ggpubr)
curr <- countrycode::codelist[which(countrycode::codelist$iso.name.en=="India"),]$iso4217c

  ggplot(df)+
  geom_histogram(aes(x=avg_farm_gate_price),fill=base_col,col=base_col,binwidth = 0.2)+
  scale_x_continuous(name=paste0("Farm gate price in\n",curr," / 100 kg"))+
  theme_classic2()+
  theme(text = element_text(size = 30))
```  
