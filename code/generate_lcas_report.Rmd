...
m1 <- mapview(sf["harvest_year"],alpha.regions = 0.3,cex=0.1,legend=F)
mapshot(m1,"../outputs/m1.html",selfcontained = FALSE)

```

<iframe src="../outputs/m1.html" data-external="1"></iframe>
...
means <- unlist(lapply(list_points,
       function(x) {
         mean(sf[x,]$ton_per_hc,na.rm=T)
       }
))
sf_aoi_cropped$mean_yield <- means
sf_aoi_omit <- sf_aoi_cropped[!is.na(sf_aoi_cropped$mean_yield),]
library(RColorBrewer)
#mapview(sf["ton_per_hc"],cex=0.5,alpha=0.01,legend=F)+
  mapview(sf_aoi_omit,zcol="mean_yield",
          col.regions = brewer.pal(4, "YlGn")
          )
...
- Histrogram of each + maps of each (e.g. district average map). Print mean (sd) for each district as a label.
